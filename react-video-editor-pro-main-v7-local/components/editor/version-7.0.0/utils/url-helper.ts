...
export const resolveMediaUrl = (url: string, baseUrl?: string): string => {
  ...
  // In any other context, use the toAbsoluteUrl function
 ...
 * @returns A promise that resolves to true if the file is accessible
 */
export const isFileAccessible = async (url: string): Promise<boolean> => {
  try {
    ...
    console.error(`Failed to check file accessibility for ${url}:`, error);
    return false;
  }
};
